...
extern "C" {
    ...
    //     float elapsedTime_g;
    //     cudaEventElapsedTime(&elapsedTime_g, start, stop); // 計算時間�?
    ...
    EXPORT void load_Beamline_From_DatFile(char* filename, double ReferenceEnergy = 100){
        ...
        for(int i = 0; i < input_dat.size(); ++i){
            ...
            {
                // 在TraceWin里，EDGE-BEND-EDGE这样的组合表示偏转铁，单位需要注�?
...
//    EXPORT uint get_element_index(string* str){
//        return beamline_->GetElementModelIndex(str);
//    }
//
//
//    EXPORT char* get_name_by_index(uint index){
//        return beamline[index];
//    }
//
//    // for quad
//    EXPORT void change_quad_gradient(uint index, double change_value){

          // 假设你已经将 Dipole 对象添加�? BeamLine �?
//          BeamLineElement* tmp = BeamLine[index];  // 获取第一个元�?
//
//// 尝试将其转换�? Dipole 类型
//Quad* quad = dynamic_cast<Quad*>(tmp);
//
//if (quad) {
//    // 如果转换成功，调�? Dipole 类的 SetGradient 函数
//    quad->SetGradient(5.0);  // 假设 5.0 是你要设置的 gradient
//} else {
//    // 如果转换失败，说明这个元素不�? Dipole 类型
//    std::cout << "Element is not a Quad!" << std::endl;
//}


//        beamline[index]->SetGradient(change_value);
//    }
//    // for dipole
//    EXPORT void change_dipole_Radius(uint index, double change_value){
//        beamline[index]->SetRadius(change_value);
//    }
//
//    EXPORT void change_dipole_Angle(uint index, double change_value){
//        beamline[index]->SetAngle(change_value);
//    }
//
//    EXPORT void change_dipole_EdgeAngleIn(uint index, double change_value){
//        beamline[index]->SetEdgeAngleIn(change_value);
//    }   
//
//    EXPORT void change_dipole_SetEdgeAngleOut(uint index, double change_value){
//        beamline[index]->SetEdgeAngleOut(change_value);
//    }
//    // for ApertureRectangular
//    EXPORT void change_ApertureRectangular_XLeft(uint index, double change_value){
//        beamline[index]->SetApertureXLeft(change_value);
//    }
//
//    EXPORT void change_ApertureRectangular_XRight(uint index, double change_value){
//        beamline[index]->SetApertureXRight(change_value);
//    }
//
//    EXPORT void change_ApertureRectangular_YTop(uint index, double change_value){
//        beamline[index]->SetApertureYTop(change_value);
//    }
//
//    EXPORT void change_ApertureRectangular_YBottom(uint index, double change_value){
//        beamline[index]->SetApertureYBottom(change_value);
//    }
//    //  share
//    EXPORT void change_length_by_index(uint index, double change_value){
//        beamline[index]->SetLength(change_value);
//    }
//
////     get_value for test
//    EXPORT double get_length_by_index(uint index){
//        return beamline[index]->GetLength();
//    }
    ...
    EXPORT char* simulate_and_getEnvelope_CPU(bool use_spacecharge){
        ...
        position_arr.push_back(position);
        avgx_arr.push_back(getAvgX());     #  画图程曦
        avgy_arr.push_back(getAvgY());
        avgxp_arr.push_back(getAvgXp());
        avgyp_arr.push_back(getAvgYp());
        sigx_arr.push_back(getSigX());
